...
def _build_derived_status_results(
    ...
    for resolved_status in resolver.get_all_resolved_derived_statuses():
        ...
        if not resolved_ids:
            print(f"[derived-status] '{status_label}': No matching expectations, skipping")
            continue

        # Concise logging - just show what matters
        resolution_mode = resolved_status.get("resolution_mode", "unknown")
        print(f"[derived-status] '{status_label}': Evaluating {len(resolved_ids)} expectation(s) [mode: {resolution_mode}]")
        ...
        if unexpected_count == 0:
            print(f"[derived-status] '{status_label}': No failures found, skipping")
            continue

        total_failure_records = sum(counts_map.get(exp_id, 0) for exp_id in resolved_ids)
        print(f"[derived-status] '{status_label}': {unexpected_count} unique materials ({total_failure_records} total failures)")
