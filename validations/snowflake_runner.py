...
def _parse_sql_results(df: pd.DataFrame, suite_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for validation in validations:
        ...
        if val_type == "expect_column_values_to_not_be_null":
            results.extend(_parse_not_null_results(df, validation))
        elif val_type == "expect_column_values_to_be_in_set":
            results.extend(_parse_value_in_set_results(df, validation))
        elif val_type == "expect_column_values_to_not_be_in_set":
            results.extend(_parse_value_not_in_set_results(df, validation))
        elif val_type == "expect_column_values_to_match_regex":
            results.extend(_parse_regex_results(df, validation))
        elif val_type == "expect_column_pair_values_to_be_equal":
            results.append(_parse_column_pair_equal_result(df, validation))
        elif val_type == "expect_column_pair_values_a_to_be_greater_than_b":
            results.append(_parse_column_pair_greater_result(df, validation))
        elif val_type == "custom:conditional_required":
            results.append(_parse_conditional_required_result(df, validation))
        elif val_type == "custom:conditional_value_in_set":
            results.append(_parse_conditional_value_in_set_result(df, validation))
...
def _parse_not_null_results(df: pd.DataFrame, validation: Dict) -> list:
    ...
    for col in columns:
        ...
        context_cols = get_context_columns_for_columns([col])
        failed_materials = _build_failure_records(
            df, flag_col, context_cols, extra_fields={"Unexpected Value": col}
        )
...
def _parse_value_in_set_results(df: pd.DataFrame, validation: Dict) -> list:
    ...
    for column, allowed_values in rules.items():
        ...
        context_cols = get_context_columns_for_columns([column])
        failed_materials = _build_failure_records(
            df, flag_col, context_cols, extra_fields={"Unexpected Value": column}
        )
...
def _parse_value_not_in_set_results(df: pd.DataFrame, validation: Dict) -> list:
...
def _parse_regex_results(df: pd.DataFrame, validation: Dict) -> list:
...
def _parse_column_pair_equal_result(df: pd.DataFrame, validation: Dict) -> Dict:
    ...
    context_cols = get_context_columns_for_columns([col_a, col_b])
    failed_materials = _build_failure_records(
        df, flag_col, context_cols, extra_fields={col_a: col_a, col_b: col_b}
    )
...
def _parse_column_pair_greater_result(df: pd.DataFrame, validation: Dict) -> Dict:
...
def _parse_conditional_required_result(df: pd.DataFrame, validation: Dict) -> Dict:
    ...
    context_cols = get_context_columns_for_columns([condition_col, required_col])
    failed_materials = _build_failure_records(
        df,
        flag_col,
        context_cols,
...
def _parse_conditional_value_in_set_result(df: pd.DataFrame, validation: Dict) -> Dict:
    ...
    context_cols = get_context_columns_for_columns([condition_col, target_col])
    failed_materials = _build_failure_records(
        df,
        flag_col,
        context_cols,
